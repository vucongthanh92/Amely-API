...
function updateEAV($db, $object, $show_id = false)
{
	...
	foreach ($arr as $ksubtype => $val) {
		...
		saveTableMetaData($update, $entities_metadata, "update");
	}
	return true;
}

function insertEAV($db, $object, $show_id = false)
{
	$object_guid = insertTableObject($db, $object, true);
	if ($object_guid) {
		foreach ($object->data as $key => $value) {
			...
			$entities_guid = saveTableEntities($db, $entities, true);
			$entities_metadata = new stdClass();
			$entities_metadata->guid = $entities_guid;
			$entities_metadata->value = $value;
			saveTableMetaData($db, $entities_metadata);
		}
	}
	if ($show_id) {
		return $object_guid;
...
function saveTableObject($db, $object, $action = "insert", $show_id = false) 
{
...
function insert($db, $params, $show_id = false )
{
...
function update($db, $params, $show_id = false )
{
...
function getData($db, $table, $conditions, $offset = 0, $limit = 10, $load_more = true)
{
